...
scripts/
  ...
  02_verify_poster_exists.py Does this row have a reachable poster at all?                   [TMDB only]
  03_fetch_alt_titles.py     Alternate titles: TMDB API + optional IMDb AKAs                 [TMDB only]
  04_bedrock_ocr.py          Vision-LLM (Nova Pro) reads the poster directly                 [needs AWS]
  05_comprehend_language.py  Language of the poster's visible text                           [needs AWS]
  06_translate_titles.py     Re-score non-English text after translation                     [needs AWS]
  07_dedupe_tmdb_metadata.py Same title+year+overview, different id?                         [TMDB only]
  08_dedupe_poster_md5.py    Same exact poster image file used twice?                        [TMDB only]
  09_collapse_compilations.py Same poster shared across multiple ids?                        [TMDB only]
  10_validate_corpus.py      Orchestrates 1-9, writes final outputs                    [needs AWS, 04-06]
  11_find_alternate_posters.py Discover + download TMDB poster variants                      [TMDB only]
  ...
  10_validate_corpus.py's own gates 7-9 sub-steps are TMDB-only; it needs
