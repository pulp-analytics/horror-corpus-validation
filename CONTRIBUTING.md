...
shipped backwards more than once. Real example: `11_collapse_compilations.py`
...
`12_validate_corpus.py`'s `compute_dedup_exclusions()` and
...
the commit that added `compute_dedup_exclusions()`: gate 10 ran before
gate 11 purely by code order, and on a real case that made it produce the
wrong answer.
